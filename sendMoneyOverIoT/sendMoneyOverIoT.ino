...
const char* host = "resources.benax.rw"; //domain or subdomain of your website
...
void setup(){
  ...
  connectToWiFi("Benax-WiFi(2.4G)", "Rc@Nyabihu2023");
  SPI.begin();                                                 
  mfrc522.PCD_Init();                                                
}
...
void connectToHost(){
  ...
  while((!wifiClient.connect(host, 80)) && (retry_counter <= 30)){
    delay(100);
    Serial.print("connecting to remote server.");
    retry_counter++;
  }
